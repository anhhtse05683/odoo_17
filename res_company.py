...
class Company(models.Model):
    ...
    def _get_date_range(self, years=False):
        ...
        if not years:
            years = 0
        first_day = fields.Date.today() - timedelta(days=0)
        first_day = start_of(subtract(first_day, years=years),self.manufacturing_period)
    ...
    def _date_range_to_str(self):
        ...
        for date_start, date_stop in date_range:
            if self.manufacturing_period == 'month':
                ...
                dates_as_str.append(_('W%(week_num)s (%(start_date)s-%(end_date)s/%(month)s)',
